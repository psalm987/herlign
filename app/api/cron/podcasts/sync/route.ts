 ...
 * 
 * Weekly cron job to sync podcasts from YouTube
 */
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (authHeader !== `Bearer ${process.env.IP_HASH_SALT}`) {
        ...
        // Add new videos (is_visible defaults to true)
        ...
        if (existingPodcasts && (existingPodcasts as Podcast[])?.length) {
            for (const podcast of (existingPodcasts as Podcast[])) {
                if (!youtubeVideoIds.has(podcast.youtube_video_id)) {
                    const { error } = await supabase
                        .from('podcasts')
                        .delete()
                        .eq('id', podcast.id);
                    ...
                    if (!error) {
                        ...
                        console.error(`Failed to remove video ${podcast.youtube_video_id}:`, error);
                    }
                }
            }
        }

        console.log(`Podcast sync completed: ${added} added, ${updated} updated, ${removed} removed`);
        ...
        console.error('Cron sync podcasts error:', error);
